...
class QwenVL3_image:
    def __init__(self):
        ...
        print(f"QwenVL3_image Node Initialized. ID: {id(self)}. Device: {self.device_info['device_type']}")
    ...
    RETURN_TYPES = ("STRING", "QWEN_PIPE",)
    RETURN_NAMES = ("text", "qwen_pipe",)
    FUNCTION = "process"
    CATEGORY = "Qwenvl"
...
class QwenVLUnloadModel:
    ...
    def unload(self, qwen_pipe):
        print(f"\n{'='*60}")
        print(f"[QwenVLUnloadModel] Unload node triggered")
        print(f"[QwenVLUnloadModel] Received pipe: {qwen_pipe}")
        
        instance = qwen_pipe.get("qwen_instance")
        print(f"[QwenVLUnloadModel] Extracted instance: {instance}")
        print(f"[QwenVLUnloadModel] Instance ID: {id(instance) if instance else 'None'}")

        if instance and hasattr(instance, 'clear_model_resources'):
            print(f"[QwenVLUnloadModel] Calling clear_model_resources on instance {id(instance)}")
            instance.clear_model_resources()
            print(f"[QwenVLUnloadModel] Unload completed successfully")
        else:
            print(f"[QwenVLUnloadModel] ERROR: Could not find valid instance or clear_model_resources method")
        
        print(f"{'='*60}\n")
        return ()

NODE_CLASS_MAPPINGS = {
    "QwenVL3_image": QwenVL3_image,
    "QwenVLUnloadModel": QwenVLUnloadModel,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "QwenVL3_image": "QwenVL3 Image (FP8)",
    "QwenVLUnloadModel": "QwenVL Unload Model",
}